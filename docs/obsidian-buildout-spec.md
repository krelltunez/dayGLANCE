...
3. **Arbitration and handoff** — `pluginAuthoritative` consumed, direct write paths stop per 3.2, the Settings mode indicator, and the handoff hazards closed. As built, with the two gate rulings:
   - **Gate (a) — the writeback snapshot: EMIT-IN-SAME-TICK.** On an authoritative device the intent emission (already firing in the same tick since the stream PR) *is* the write — the direct writer is simply not called. There is no "skipped write" state for the snapshot to advance past: every detected change produces exactly one action, so every snapshot advance is backed by an emission. A failed *enqueue* reuses the direct writer's latch discipline (visible error, retried when the task next changes — the same surface a failed direct write has always had), and `emitBridgeIntent` reports enqueue failure honestly (a swallowed storage error must not report "queued"). The write-success-gated id/rawTitle/retirement bookkeeping does NOT run at emit time: "success" in plugin mode is the vault line coming back as an observation, which the existing adoption machinery absorbs exactly like another device's write. The stale-authority window (authority is refreshed once per cycle) is safe by construction: both writers produce byte-identical output (the convergence pins).

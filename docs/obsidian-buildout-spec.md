...
3. **Arbitration and handoff** — `pluginAuthoritative` consumed, direct write paths stop per 3.2, the Settings mode indicator, and the handoff hazards closed. As built, with the two gate rulings:
   ...
   - **Gate (b) — the detector baseline: CLEARED ON EVERY TRANSITION, BOTH DIRECTIONS** (`utils/obsidianBridgeMode.js`), dates sidecar in lockstep. An empty baseline is itself the one conservative no-detection cycle — the first post-transition scan diffs against nothing, reports nothing, and establishes a fresh baseline; detection resumes on the second. Clearing on the direct→plugin edge too means nothing can fossilize regardless of how the device later leaves plugin mode. Checked for other fossils: tombstones deliberately KEPT (real observed deletions, LWW-revivable, retention-pruned); wikilink candidates cosmetic; the writeback snapshot is gate (a)'s; the Tasks-plugin detection still refreshes (reads continue in plugin mode — only writes and the scan stop). Mode can flap with Obsidian's own liveness (§3.3's one revert path) — each flap costs one conservative cycle, the cheap side of the trade.
   - One inbound source per device: authoritative → the observation stream (through the shared scan pipeline, cursor committed after apply), never the direct scan; anything else → the scan. **Accepted cost, recorded:** while a device is in plugin mode it detects no vault deletions (observations cannot establish scan completeness) — tasks linger rather than tombstone, and detection resumes with direct mode.

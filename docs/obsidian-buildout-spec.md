...
- **Android has no timer: writes arm the launcher, leaving the app fires it.** Two forcing reasons, recorded so this is not relitigated. UX: an intent fired while the user is inside dayGLANCE brings Obsidian to the foreground and interrupts their flow — there is no Android equivalent of macOS's background `activate: false`. Correctness: since Android 10 the OS blocks activity starts from apps no longer in the foreground (the background-activity-launch restriction), so a timer expiring after the user left would be silently discarded — a timer-based Android launch only works when it interrupts, and fails exactly when the user edits and puts the phone away. App exit, hooked while the activity is still foreground, is the only moment that is both non-interruptive and reliably permitted. The exit is the debounce: one launch per session-with-writes, opening the last-written note. Screen-off and incoming calls deliberately do not fire (`onUserLeaveHint` excludes them); the armed state survives until a real exit and dies with the process.
...
| Android | `ACTION_VIEW` from the Kotlin bridge (existing `ObsidianBridge.openNote` path), **armed by vault writes and fired on app exit** — Home/Recents (`onUserLeaveHint`) or back-button/gesture exit (the back callback), both while the activity is still foreground. **Not Capacitor** — `dayglance-android` is a native Kotlin WebView app with `@JavascriptInterface` bridges. No Tasker needed | Off |

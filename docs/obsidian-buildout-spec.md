...
3. **Arbitration and handoff** — `pluginAuthoritative` consumed, direct write paths stop per 3.2, the Settings mode indicator, and the handoff hazards closed. As built, with the two gate rulings:
   - **Gate (a) — the writeback snapshot: EMIT-IN-SAME-TICK, with the id bookkeeping riding the enqueue.** **This is a corrected ruling — as with gate (b), the correction itself is the record.** On an authoritative device the intent emission (already firing in the same tick since the stream PR) *is* the write — the direct writer is simply not called. There is no "skipped write" state for the snapshot to advance past: every detected change produces exactly one action, so every snapshot advance is backed by an emission. A failed *enqueue* reuses the direct writer's latch discipline (visible error, retried when the task next changes — the same surface a failed direct write has always had), and `emitBridgeIntent` reports enqueue failure honestly (a swallowed storage error must not report "queued"). All of that stands. **What was first claimed and shipped:** the write-success-gated id/rawTitle/retirement bookkeeping did NOT run at emit time — "success" in plugin mode was the vault line coming back as an observation, which the existing adoption machinery would absorb "exactly like another device's write." **Why that was wrong:** the claim holds only for ^dg--tagged tasks, where the block token carries identity whatever the title says. For a LEGACY-ID task being retitled the id is content-derived: the applied line's parse and its legacy-id hint both derive from the NEW title, and a real other device would have recorded the retirement at its own write time — here nobody ever did. The old-id copy lingered in app state, the renamed line imported as a second task, and the cross-list reconciler fought the resupply every cycle at SSE speed: the live reconcile war of 2026-08-30 (#1455's second instance), initially misdiagnosed as a deletion loop — the user had deleted nothing; they renamed a task. **What replaced it:** in plugin mode the retitle's commit() — titleUpdate, block-id assignment, retirement recording — runs on SUCCESSFUL ENQUEUE, deferred exactly like a confirmed native write. Gate (a)'s principle is that every snapshot advance is backed by exactly one action, and the id bookkeeping rides that same action: the outbox is durable, so enqueue is the write in every sense the snapshot already trusts. The trade, weighed not missed: an enqueued intent later lost (revoked pairing, outbox overflow) leaves the app briefly holding an id no vault line carries — bounded by the same latch-and-surface discipline, and a lost retitle already leaves visible divergence today. The rejected alternative — teaching the observation side to bridge old ids — stays rejected: it means carrying rename semantics in observations, which §3.6 deliberately forbids. Non-retitle intents still settle through the observation round-trip (the ^dg- token, or the unchanged-title legacy hint, bridges them), so only the case that was broken changed. Pinned end to end by the rename-war reproduction (`useObsidianSync.renameWar.test.js`). The stale-authority window (authority is refreshed once per cycle) remains safe by construction: both writers produce byte-identical output (the convergence pins).

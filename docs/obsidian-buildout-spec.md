...
2. **Tasks-emoji + Dataview-inline READ, together, next.** One trailing-metadata extraction over both serializations (the Tasks plugin itself has a dataview-format mode, so reading only emoji misses half its users), under the IDENTITY-FROZEN design: `obsidianRawTitle` keeps the full line text (identities, write matching, and metadata preservation through rewrites stay byte- and version-stable) and only the display title is stripped; mapping due→deadline, scheduled→inline-date, priority→0–3, ✅→completedAt; 🔁 recurrence recognized but NOT mapped (Tasks' next-instance-on-completion model doesn't fit dayGLANCE's recurring tasks). Two items must be settled in that step: the retitle-metadata-carry wrinkle (a DG rename derives the written title from the display title and would drop the metadata), and the 🔁-completion ownership question (dayGLANCE's writeback already checks 🔁 lines today without minting the next instance — whether to decline/warn/keep is a write-behavior decision to be made explicitly).

...
**A reset on one device does not clear a fleet.** Deleting the file is necessary but not sufficient while another device is running: the reset device seeds a fresh empty snapshot (its sync record was wiped along with localStorage), the other device merges that empty snapshot — `mergeArrayById` keeps every local-only row when the remote is empty and carries no tombstones — and writes its full dataset back. Note the lever is the **merge**, not the seed: the `onIOS &&` guard in `App.jsx` `iCloudSync` looks like the culprit but tightening it changes nothing, because step three is a merge. Propagating a wipe would need a reset tombstone in the payload (a `resetAt` other devices honour by wiping instead of merging) — a new destructive broadcast primitive in an engine deliberately biased toward keeping data (see `sync/snapshotDeleteGuard.js`), so it is a considered change rather than a patch. Quitting the other devices is not a workaround: a closed device still holds its full localStorage and republishes the moment it is reopened, so quitting only avoids the immediate race. Note the obvious phrasing — "reset every device before reopening any of them" — is circular, since resetting a device requires opening it. The workable procedure is one device at a time: quit dayGLANCE everywhere, open a single device and reset it (it republishes in the second before the user can click, which is harmless with nothing else running to receive it, and the reset deletes the file afterwards), quit it, then move to the next. What breaks it is two unreset devices running at once, or reopening an unreset device after cleaning another.

Neither platform offers a clean OS-level route either. On iOS it takes two separate actions — delete the app (sandbox only) AND Settings → Apple ID → iCloud → Manage Account Storage → dayGLANCE → Delete Data (container only); doing just the first is the bug this whole section describes. macOS has no route at all: the app never registers with the iCloud daemon so there is no per-app control, and moving dayGLANCE.app to the Trash leaves `~/Library/Application Support/dayGLANCE` (set explicitly in electron/main.ts) and the container in `~/Library/Mobile Documents/iCloud~com~dayglance` both intact.
...
- **Bonus data available**: HRV, resting heart rate, blood oxygen, and respiratory rate can be surfaced in a richer sleep card without any web-layer changes beyond reading the extra fields
...
**Status: ✅ Complete.** Minor gap: status bar text colour is static (`UIStatusBarStyleDefault`). It does not flip to white in dark mode. Cosmetic; can be addressed in Phase 12 polish.
...
- **iOS**: `CloudSyncBridge.swift` exposes `nativeGetCloudData()`, `nativeSetCloudData(json)`, `nativeIsCloudSyncAvailable()`; `NSMetadataQuery` watch for remote changes
- **macOS (Electron)**: `electron/icloud-sync.ts` handles file read/write at ubiquity container path; `fs.watch` for remote changes; `icloud:read` / `icloud:write` IPC handlers
...
- **Reminders read (opt-in)**: EventKit already covers Reminders. Add a Settings toggle "Show iOS Reminders as inbox tasks" (off by default). When enabled, reminders surface read-only alongside dayGLANCE inbox tasks. This gives Apple-ecosystem users a way to consolidate without forced migration.

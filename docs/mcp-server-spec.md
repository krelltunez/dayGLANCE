...
- **Settings-cluster button in the main window** (canonical, all platforms): rendered only while the listener is bound — a visible bolt IS the ambient signal; an absent bolt is the off state. Status dot per the settings-cluster convention: green (enabled, no undoable writes), blue (undoable MCP writes exist), red (writes auto-disabled after repeated rate-limit violations, §4.3). Clicking expands an inline panel below the header row — in document flow, never a popover — with server status, current tier, the one-click kill switch, the journal entry count, and bulk undo. The narrow-viewport variant is a card section in the mobile Settings tab — the mobile layout's home for secondary status and controls — because the same layout renders in Electron windows under 721px wide. It deliberately does NOT sit in the date-nav header, the layout's most contended row.

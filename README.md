...
All-day items — a holiday, a birthday, anything without an hour — never go on the ring, since nothing timeless belongs on a time axis. They sit in their own pill beside the day's totals at the bottom, in the same grammar as the rest of that band: on a landscape display that space is already empty, so they cost no dial diameter, and on a phone the pill stacks above the totals instead. Each one opens the same actions as a scheduled block, and the pill collapses to a quiet "+N" when the day carries more than fit.

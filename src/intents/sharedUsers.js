...
const GLANCE_DIR = '/GLANCE/';

function glanceDir(config) {
  const base = (config.webdavUrl ?? '').replace(/\/+$/, '');
  return `${base}${GLANCE_DIR}`;
}
...
/**
 * Sync the local user list with GLANCE/glance-users.json on WebDAV.
